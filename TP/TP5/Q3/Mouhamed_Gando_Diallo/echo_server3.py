...
def server(readfd):
	...
	#while True: # Laisse le server actif meme après la déconnexion de tous les clients
		...
		while True:
			...
			if text.startswith(_SEPARATOR):
				...
				if text == '':
					...
					continue
				for clien in writefds:
					# if clien == client:  # condition pour pas que le client émetteur reçoit son propre message
					# 	continue
					os.write(writefds[clien], text.encode('utf-8'))
